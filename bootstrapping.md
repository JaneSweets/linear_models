...
The result is counterintuitive - the std.error for the intercept should
be lower for the nonconst model. In this kind of setting where we can’t
trust the lm results (because the hypotheses do not hold for the
nonconst data), we try bootstrapping.
...
The se for intercept decreases while the se for x increases, which shows
that the results derived from bootstrapping are more plausible.
...
can I construct a CI (need larger sample size)
